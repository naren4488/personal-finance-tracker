...
function AdjustBalanceSheetMounted({ account, onOpenChange }: MountedProps) {
  const titleId = useId()
  ...
  return (
    <div className="fixed inset-0 z-70 flex min-h-0 max-h-dvh items-center justify-center overflow-hidden p-2 pb-[max(0.75rem,env(safe-area-inset-bottom))] sm:p-4">
      ...
      >
        ...
        >
          <div className={cn(FORM_OVERLAY_SCROLL_BODY, "space-y-3 px-4 py-2")}>
...
export function AdjustBalanceSheet({ open, onOpenChange, account }: AdjustBalanceSheetProps) {
  ...
  return <AdjustBalanceSheetMounted account={account} onOpenChange={onOpenChange} />
}
